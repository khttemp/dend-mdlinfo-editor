...
class TreeViewDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        self.mdlInfoLf = LabelFrame(mainFrame, text="モデル詳細情報")
        ...
        for colName in self.colTuple:
            ...
            if colName == "ele1-3":
                ...
                widthLen = 150
            self.tree.column(colName, anchor=CENTER, width=widthLen)
            self.tree.heading(colName, text=colName, anchor=CENTER)
...
class TexImageDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        self.imageLb = Label(self.listFrame, font=("", 14), text="イメージリスト")
        self.imageLb.grid(row=0, column=0, sticky=W+E)
        self.v_imageList = StringVar(value=self.txtImgList)
        self.imageListbox = Listbox(self.listFrame, font=("", 14), listvariable=self.v_imageList, width=30)
        self.imageListbox.grid(row=1, column=0, sticky=W+E)
    ...
    def insert(self):
        ...
        if result:
            self.dirtyFlag = True
            self.imageListbox.insert(END, result)
    ...
    def delete(self):
        ...
        if result:
            self.dirtyFlag = True
            self.imageListbox.delete(self.selectIndex)
            self.imageListbox.select_set(END)
...
class DetailDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        for colName in self.colTuple:
            if colName == "mdlNo":
                self.imageLb = Label(master, font=("", 14), text="モデル{0}-{1}の要素".format(self.textureList[0], self.textureList[1]))
                self.imageLb.grid(row=index, column=0, columnspan=2, sticky=W+E)
                index += 2
            elif colName == "ele1-3":
                for i in range(3):
                    self.eleLb = ttk.Label(master, font=("", self.fontSize), text="ele{0}".format(i+1))
                    self.eleLb.grid(row=index, column=0, sticky=W+E)
                    self.v_ele = IntVar()
                    ...
                    self.eleEt.grid(row=index, column=1, sticky=W+E)
...
class ImageDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        self.imageLb = Label(self.listFrame, font=("", 14), text="イメージリスト")
        self.imageLb.grid(row=0, column=0, sticky=W+E)
        self.v_imageList = StringVar(value=self.imgList)
        self.imageListbox = Listbox(self.listFrame, font=("", 14), listvariable=self.v_imageList, width=30)
        self.imageListbox.grid(row=1, column=0, sticky=W+E)
        self.imageListbox.bind('<<ListboxSelect>>', lambda e:self.buttonActive(e, self.imageListbox.curselection()))
...
class SmfDetailDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        self.mdlInfoLf = LabelFrame(mainFrame, text="smf要素情報")
    ...
    def modify(self):
        ...
        if result.dirtyFlag:
            self.reload()
            if self.selectIndex != None and self.selectIndex >= 0:
...
class SmfDetailEditDialog(sd.Dialog):
    ...
    def body(self, master):
        ...
        index = 0
        if self.valueList == None:
            self.position = ttk.Label(master, font=("", self.fontSize), text="挿入する位置")
            self.position.grid(row=index, column=0, sticky=N+S)
            if self.smfDetailNum != None:
            ...
            self.positionCb.grid(row=index, column=1, sticky=N+S, pady=10)
            self.v_position.set(positionList[0])
            index += 1

            self.xLine = ttk.Separator(master, orient=HORIZONTAL)
            self.xLine.grid(columnspan=2, row=index, column=0, sticky=E+W, pady=10)
            index += 1
